...
# API ключи
TELEGRAM_TOKEN = "8385597047AAFdgzjzXd52C2NSScipGzIpZyiOGrpSdyY"
...
async def enhance_image_prompt(user_prompt)
    ...
    try
        response = requests.post(
            f{AITUNNEL_URL}chatcompletions,
            headers={Authorization fBearer {AITUNNEL_KEY}, Content-Type applicationjson},
            json={
                model gpt-4o-mini,
                messages [
                    {
        ...
        logger.info(fEnhanced prompt {enhanced})
        return enhanced
    except Exception as e
        logger.error(fPrompt enhance error {e})
        return user_prompt
...
async def generate_image_genapi(prompt, model=flux-2)
    ...
    try
        response = requests.post(
            ...
            headers={
                Authorization fBearer {GENAPI_KEY},
        ...
        logger.info(fGenAPI response {result})
        ...
        if result.get(status) == SUCCESS
            ...
            raise Exception(fNo image URL in response {result})
        
        if not image_url
            raise Exception(fEmpty image URL {result})
        ...
        logger.error(fGenAPI generation error {e})
        raise
...
async def edit_image_genapi(image_bytes, prompt, model=flux-2)
    ...
    image_bytes BytesIO с изображением
    prompt инструкция для редактирования
    ...
    try
        ...
        # Формируем промпт для сохранения оригинала
        edit_instruction = (
            fKeep the original image composition, lighting, and all unchanged elements exactly as they are. 
            fOnly apply this specific change {prompt}. 
            fPreserve everything else without modifications.
        )
        ...
        response = requests.post(
            ...
            json={
                ...
                image fdataimagepng;base64,{image_base64},  # Передаём изображение
                translate_input True,
                is_sync True
            },
            timeout=120
        ...
        result = response.json()
        logger.info(fGenAPI edit response {result})
        ...
        logger.error(fGenAPI edit error {e})
        raise
...
async def start(update Update, context ContextTypes.DEFAULT_TYPE)
    ...
    await update.message.reply_text(
        🤖 AI Бот - чат и генерация!nn
        💬 Чат с AIn
        🎨 Генерация изображений (Flux 2)n
        🖼️ Редактирование фотоnn
        Free 10 запросовденьn
...
async def handle_buttons(update Update, context ContextTypes.DEFAULT_TYPE)
    ...
    if text == 💎 Купить Premium
        ...
        chat_mode.pop(user_id, None)
        await update.message.reply_text(
            f💎 Premium подписка - 200₽месяцnn
            f✨ Безлимитный чат с AIn
            f🎨 Генерация изображений (Flux 2)n
            f🖼️ Редактирование фотоnn
            fВаш ID {user_id}nn
            fДля активации напиши администратору,
            reply_markup=main_keyboard(user_id)
        )
        return
    ...
    if text == 💬 Чат с AI
        ...
        await update.message.reply_text(
            💬 Режим чата включен!nnПиши свои вопросы, 
    ...
    if text == 🎨 Генерация
        ...
        if not is_premium
            await update.message.reply_text(
                🔒 Генерация только для Premium!nnОбратись к администратору.,
                reply_markup=main_keyboard(user_id)
            )
            return
        chat_mode[user_id] = image
        await update.message.reply_text(
            🎨 Режим генерации (Flux 2)!nnОпиши картинку, 
    ...
    if text == 👑 Админ панель and user_id == ADMIN_ID
        ...
        chat_mode.pop(user_id, None)
        await update.message.reply_text(👑 Админ панельnnВыбери действие, reply_markup=admin_keyboard())
        return
    ...
    if user_id == ADMIN_ID
        if text == ➕ Выдать Premium
            ...
            await update.message.reply_text(
                ➕ Выдать PremiumnnОтправь @username или ID пользователя, 
        ...
        if text == ➖ Забрать Premium
            ...
            await update.message.reply_text(
                ➖ Забрать PremiumnnОтправь @username или ID пользователя, 
                reply_markup=admin_keyboard()
            )
            return
        ...
        if text == 🚫 Заблокировать
            ...
            await update.message.reply_text(
                🚫 Заблокировать пользователяnnОтправь @username или ID, 
                reply_markup=admin_keyboard()
            )
            return
        ...
        if text == ✅ Разблокировать
            ...
            await update.message.reply_text(
                ✅ Разблокировать пользователяnnОтправь @username или ID, 
                reply_markup=admin_keyboard()
            )
            return
        ...
        if text == 📋 Список Premium
            ...
            text_list = n.join([f• {uid} (@{uname or 'нет'}) for uid, uname in users]) if users else Список пуст
            await update.message.reply_text(
                f🌟 Premium пользователи ({len(users)})nn{text_list}, 
                reply_markup=admin_keyboard()
            )
            return
        ...
        if text == 📊 Статистика
            ...
            await update.message.reply_text(
                f📊 Статистика ботаnn
                f👥 Всего пользователей {total}n
                f🌟 Premium {premium}n
                f🔒 FREE {total - premium}n
...
async def handle_text(update Update, context ContextTypes.DEFAULT_TYPE)
    ...
    if user_id == ADMIN_ID and user_id in admin_mode and admin_mode[user_id] not in [main]
        ...
        try
            if text.startswith('@')
                ...
                if not target_id
                    await update.message.reply_text(
                        f❌ Пользователь не найден!nnПользователь должен запустить бота start, 
                        reply_markup=admin_keyboard()
                    )
                    return
            else
            ...
            if action == grant
                ...
                await update.message.reply_text(f✅ Premium выдан!nnПользователь ID {target_id}, reply_markup=admin_keyboard())
            elif action == revoke
                set_premium_status(target_id, 0)
                await update.message.reply_text(f✅ Premium удалён!nnПользователь ID {target_id}, reply_markup=admin_keyboard())
            elif action == block
                block_user(target_id)
                await update.message.reply_text(f🚫 Пользователь заблокирован!nnID {target_id}, reply_markup=admin_keyboard())
            elif action == unblock
                unblock_user(target_id)
                await update.message.reply_text(f✅ Пользователь разблокирован!nnID {target_id}, reply_markup=admin_keyboard())
            ...
            await update.message.reply_text(
                ❌ Неверный формат!nnОтправь числовой ID или @username, 
                reply_markup=admin_keyboard()
            )
            return
    ...
    if user_id in chat_mode and chat_mode[user_id] == image
        ...
        try
            ...
            await update.message.reply_photo(
                photo=img_data,
                caption=f🎨 {text}nn✨ Powered by Flux 2,
                reply_markup=image_keyboard()
            )
        except Exception as e
            logger.error(fImage gen error {e})
    ...
    if remaining = 0 and not is_premium
        await update.message.reply_text(
            f🔒 Лимит исчерпан!nnFREE {FREE_LIMIT} сообщений в деньn💎 Нужен Premium для безлимита,
            reply_markup=main_keyboard(user_id)
        )
        return
...
async def handle_photo(update Update, context ContextTypes.DEFAULT_TYPE)
    ...
    try
        ...
        await update.message.reply_photo(
            photo=edited_img,
            caption=f✨ {caption}nn🎨 Edited by Flux 2,
            reply_markup=kb
        )
    except Exception as e
        logger.error(fPhoto edit error {e})
